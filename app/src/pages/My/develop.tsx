...
import { Button, InfiniteScroll, Toast, Badge } from "antd-mobile";
...
export default () => {
    ...
    // 获取未读消息数量
    const { run: getUnreadCount } = useRequest(
        () => api.privateMessage.getPrivateMessageUnreadCount(),
        {
            manual: true,
            onSuccess: (response) => {
    ...
    // 前往消息页面
    const goToMessages = () => {
        history.push('/my/messages?type=message');
    };

    // 前往私信页面
    const goToPrivateMessages = () => {
        history.push('/my/messages/private');
    };
        ...
        <div className="h-full flex flex-col *:mt-8 overflow-y-auto">
            ...
            <div className="flex items-center ">
                ...
                <div className="ml-auto flex items-center">
                    <div className="text-2xl" onClick={goToPrivateMessages}>
                        <Badge content={unreadMessages > 0 ? unreadMessages : null} color="red">
                            <MessageOutline />
                        </Badge>
                    </div>
            ...
            <div className="flex items-center justify-between p-4">
                <div
                    className="text-base font-bold px-4 py-2 bg-blue-50 rounded-lg"
                ...
                <div
                    className="text-base font-bold px-4 py-2 bg-blue-50 rounded-lg"
                    onClick={goToPrivateMessages}
                >
                    私信
                    {unreadMessages > 0 && (
                        <Badge content={unreadMessages} style={{ marginLeft: 4 }} />
                    )}
                </div>
            </div>
